...
- Enrollment :: All statin users subjects who turned 75 in 2012-2014 
...
\vfill
The authors use a marginal structural model to avoid time-varying
confounding. \vfill
\hrule
\vfill
But:
...
[fn:1] Applying the roadmap of targeted learning van der Laan et al. 
...
Example: We include all diabetes patients who initiate medical treatment
...
| Treat for 2 years                           | Static               | \(1{\{t>2\}}\)               |
...
The analysis uses the information of the
time-varying covariates (comorbidity, co-medicine) to achieve a good
compromise between the available data, the desire of the
investigators, and the causal assumptions: /positivity/, /sequential coarsening at random (NUC)/, /consistency/.
...
#+begin_export latex
\begin{equation*}
\E[Y_2] = \E[\E[\underbrace{\E[\E[\underbrace{\E[Y_2|L_0,A_0,L_1,A_1]}_{R_2(L_0,A_0,L_1,A_1)}|L_0,A_0,L_1]|L_0,A_0]}_{R_1(L_0,A_0)}|L_0]]
\end{equation*}
#+end_export

- Step 1 :: \(\hat R_2\): Regress \(Y_2\) on \(L_0,A_0,L_1,A_1\)
- Step 2 :: Integrate \(\hat R_2\) with respect to \(\pi_1^*\)
- Step 3 :: \(\hat R_1\): Regress \color{blue}result of Step 2 \color{black} on \(L_0,A_0,L_1,A_1\)  
- Step 4 :: Integrate \(\hat R_1\) with respect to \(\pi_0^*\)
...
\operatorname{IF}_\P(X) = (Y_2 - R_2) \frac{\pi_1^*\pi_0^*}{\pi_1\pi_0} + 
(R_2-R_1)\frac{\pi_0^*}{\pi_0} + R_1 - \psi(\P)
\end{equation*}
#+end_export
\vfill
Targeting algorithm:
...
- Step 2 ::  \(\hat R_2\): Regress \(Y_2\) on \(L_0,A_0,L_1,A_1\)
- Step 3 ::  \(\hat R_2^*\): TMLE update: Loss function and parametric fluctuation model to solve the current part of the efficient influence function 
- Step 4 :: Integrate  \(\hat R_2^*\) with respect to \(\pi^*_1\)
...
#+begin_export latex
\begin{equation*}
\texttt{\color{red}gbounds:\hfill\color{black}}\prod_{k = 0}^K\frac{\pi_k^* 1\{C_k = 0\}}{\max(\color{red}{0.01}\color{black},\hat\pi_k \hat G_k)}\hfill
\end{equation*}
#+end_export
3. Fit model with targeted predictions as outcome conditional on past ...
...
probabilities cannot become smaller than
#+begin_export latex
\begin{equation*}
\texttt{gbounds} = 0.01.
\end{equation*}
